...
import { 
  ...
  Calendar, Clock, AlertCircle, Bell, Star, Plus, 
  Trash2, Eye, EyeOff, Globe, Server, Database,
  Shield, Wrench, FileText
} from 'lucide-react';
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (settings) {
      ...
      setFormData({
        ...settings,
        reviews: settings.reviews || DEFAULT_REVIEWS,
        stats_happy_customers: settings.stats_happy_customers || 150,
        stats_avg_rating: settings.stats_avg_rating || 4.9,
        stats_total_orders: settings.stats_total_orders || 500,
        use_real_stats: settings.use_real_stats || false,
      });
    }
  }, [settings]);
                ...
                <>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <Label>REGON</Label>
                      <Input
                        value={formData.company_regon as string || ''}
                        onChange={(e) => handleChange('company_regon', e.target.value)}
                    ...
                    <div>
                      ...
                      <Input
                        value={formData.company_bank_account as string || ''}
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Automatycznie pobieraj statystyki z bazy danych
                  </p>
                </div>
                <Switch
              ...
              {!formData.use_real_stats && (
                <div className="grid md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <Input
                      type="number"
                      value={formData.stats_happy_customers as number || 150}
                      onChange={(e) => handleChange('stats_happy_customers', parseInt(e.target.value) || 0)}
                      className="mt-1"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      min="1"
                      max="5"
                      value={formData.stats_avg_rating as number || 4.9}
                      onChange={(e) => handleChange('stats_avg_rating', parseFloat(e.target.value) || 0)}
                      className="mt-1"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      type="number"
                      value={formData.stats_total_orders as number || 500}
                      onChange={(e) => handleChange('stats_total_orders', parseInt(e.target.value) || 0)}
                      className="mt-1"
                    />
                  </div>
                </div>
              ...
              {formData.use_real_stats && (
                <div className="p-4 bg-primary/5 border border-primary/20 rounded-lg">
                  <p className="text-sm text-muted-foreground">
                    Statystyki będą automatycznie pobierane z bazy danych zamówień i użytkowników.
                  </p>
                </div>
              )}
            </CardContent>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Opinie wyświetlane na stronie głównej</CardDescription>
                </div>
              ...
              <div className="space-y-3">
                {reviews.map((review) => (
                  <motion.div
                    ...
                    className="flex items-start justify-between p-4 border rounded-lg hover:border-primary/30 transition-colors"
                  >
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {review.verified && (
                          <Badge variant="outline" className="text-xs">Zweryfikowana</Badge>
                        )}
                      </div>
                ...
                {reviews.length === 0 && (
                  <p className="text-center py-8 text-muted-foreground">Brak opinii</p>
                )}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <Input
                  type="number"
                  value={formData.estimated_delivery_days as string || '3'}
        ...
        <TabsContent value="maintenance" className="space-y-6">
          <Card className={formData.maintenance_mode === 'true' ? 'border-destructive/50' : ''}>
            <CardHeader>
              ...
              <CardDescription>
                Tymczasowo wyłącz możliwość składania zamówień i przeglądania sklepu
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Blokuje dostęp do sklepu dla niezalogowanych użytkowników
                  </p>
                </div>
                <Switch
                ...
                >
                  <div className="p-4 bg-destructive/10 border border-destructive/30 rounded-lg">
                    <p className="text-sm text-destructive font-medium">
                      ⚠️ Tryb serwisowy jest aktywny! Sklep jest niedostępny dla klientów.
                    </p>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Zalogowani klienci mogą przeglądać swoje zamówienia
                      </p>
                    </div>
                    <Switch
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <Input
                  type="email"
                  value={formData.system_email as string || ''}
                  onChange={(e) => handleChange('system_email', e.target.value)}
                  placeholder="system@layered.pl"
                  className="mt-1"
                />
              </div>
              ...
              <div>
                ...
                <Input
                  type="email"
                  value={formData.reply_to_email as string || ''}
                ...
                <SelectContent>
                  {[5, 4, 3, 2, 1].map(r => (
                    <SelectItem key={r} value={r.toString()}>
                      {r} {r === 1 ? 'gwiazdka' : r < 5 ? 'gwiazdki' : 'gwiazdek'}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
